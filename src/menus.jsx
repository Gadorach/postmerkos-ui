...
import AllPortsPanel from './all-ports';
...
function ServicePane({ client }) {
	...
	return <div className="tab-pane service-pane"><section><h3>Services</h3>{policy && ['ssh', 'web', 'chrony', 'mdns'].map(name => <fieldset key={name}><legend>{name === 'ssh' ? 'SSH' : name === 'web' ? 'Web interface' : name === 'chrony' ? 'Chrony' : 'mDNS discovery'}</legend><label className="checkbox-line"><input type="checkbox" checked={policy[name]?.enabled} onChange={event => set(name, 'enabled', event.currentTarget.checked)} /> Enabled</label><label className="checkbox-line"><input type="checkbox" checked={policy[name]?.autostart} onChange={event => set(name, 'autostart', event.currentTarget.checked)} /> Start automatically</label>{name === 'ssh' && <><label className="checkbox-line"><input type="checkbox" checked={policy.ssh.password_auth} onChange={event => set('ssh', 'password_auth', event.currentTarget.checked)} /> Allow password authentication</label><label>Listening port<input type="number" min="1" max="65535" value={policy.ssh.port} onInput={event => set('ssh', 'port', Number(event.currentTarget.value))} /></label></>}</fieldset>)}<div className="button-row"><button className="btn-primary" onClick={save} disabled={!policy}><CheckIcon /> Apply</button></div>{notice && <div className="notice">{notice}</div>}{error && <div className="error">{error}</div>}</section></div>;
}
...
function TimePane({ client }) {
	...
	const [error, setError] = useState('');
	const [notice, setNotice] = useState('');
	const [timezoneGroups, setTimezoneGroups] = useState(FALLBACK_TIMEZONE_GROUPS);
	const load = async () => {
		const response = await client.request('time_get');
		setData(response.data);
		setManualTime(manualTimeFromIso(response.data?.local));
	};
	useEffect(() => { load().catch(failure => setError(failure.message)); client.request('timezones_get').then(response => { if (Array.isArray(response.data) && response.data.length) setTimezoneGroups(response.data); }).catch(() => {}); }, [client]);
	...
	return <div className="tab-pane time-pane">
		...
		{data && <section><h3>Time zone and NTP</h3>
			<label>Common time zone<select value={selectedTimezone} onChange={event => selectTimezone(event.currentTarget.value)}><option value="custom">Custom offset and daylight-saving rule</option>{timezoneGroups.map(group => <optgroup key={group.label} label={group.label}>{group.zones.map(zone => <option key={zone.id} value={zone.id}>{zone.label} ({zone.id})</option>)}</optgroup>)}</select></label>
...
function SystemPane({ client, status }) {
	...
	const [error, setError] = useState('');
	const [resetLive, setResetLive] = useState(null);
	...
	useEffect(() => {
		...
		const poll = async () => {
			try {
				const response = await client.request('reset_button_status', undefined, { timeout: 2000 });
				if (active) setResetLive(response.data);
			} catch { /* The main connection state already reports transport errors. */ }
	...
	const reset = resetLive?.reset_button ?? status.hardware_policy?.reset_button;
	...
	const ledOwner = resetLive?.led_owner ?? status.hardware_policy?.led_owner ?? 'normal';
	...
	const release = status.release ?? {}; const services = status.services ?? {}; const discovery = status.identity ?? {}; const temperatures = status.temperature ?? status.temperatures ?? {};
	...
	return <div className="tab-pane system-pane">
		<section><h3>Device identity</h3><dl className="system-grid"><div><dt>Model</dt><dd>{status.device ?? identity.exact_model ?? '—'}</dd></div><div><dt>Product number</dt><dd>{identity.product_number ?? '—'}</dd></div><div><dt>Serial number</dt><dd>{identity.serial_number ?? '—'}</dd></div><div><dt>Base MAC</dt><dd>{identity.base_mac ?? '—'}</dd></div><div><dt>Hostname</dt><dd>{discovery.effective_hostname ?? identity.hostname ?? kernel.node ?? '—'}</dd></div><div><dt>Local discovery</dt><dd>{discovery.advertised_name ?? '—'}</dd></div><div><dt>mDNS verification</dt><dd>{discovery.advertised_name_verified ? 'verified' : 'responder-managed'}</dd></div><div><dt>mDNS conflict</dt><dd>{discovery.conflict_state ?? 'unobserved'}</dd></div><div><dt>Compatibility</dt><dd>{status.capabilities?.compatibility ?? '—'}</dd></div><div><dt>Ports</dt><dd>{status.capabilities?.port_count ?? '—'}</dd></div></dl></section>
		...
		<section><h3>Physical controls</h3><dl className="system-grid"><div><dt>Reset button</dt><dd className={`hardware-state hardware-${resetState}`}>{resetLabel}</dd></div><div><dt>Physical state</dt><dd>{reset?.available ? (reset?.pressed ? 'Pressed' : 'Released') : 'not sampled'}</dd></div><div><dt>Armed</dt><dd>{reset?.armed ? 'yes' : 'no'}</dd></div><div><dt>Input</dt><dd>{reset?.available ? `GPIO${reset.gpio} · ${reset.active_low ? 'active low' : 'active high'}` : 'not available'}</dd></div><div><dt>Safety</dt><dd>{reset?.verified && reset?.destructive_enabled ? 'hardware verified' : 'destructive action disabled'}</dd></div><div><dt>Factory reset</dt><dd>{reset?.enabled ? `hold ${reset.hold_seconds ?? 10} seconds` : 'disabled by policy'}</dd></div><div><dt>Countdown LED</dt><dd>{reset?.countdown_active ? (reset?.led_indication_active ? 'active' : 'unavailable') : 'inactive'}</dd></div><div><dt>Last button event</dt><dd>{reset?.last_event?.replace(/-/g, ' ') ?? '—'}</dd></div><div><dt>Status LED owner</dt><dd>{ledOwner}</dd></div><div><dt>Status LED</dt><dd>{statusLed?.available ? 'available' : statusLed?.verified ? 'verified but unavailable' : 'not available'}</dd></div><div><dt>Status LED evidence</dt><dd>{statusLedLabel}</dd></div></dl>{resetState === 'countdown' && <p className="warning reset-countdown-warning">Release the reset button now to cancel the factory reset.</p>}{reset?.countdown_active && !reset?.led_indication_active && <p className="warning">The reset countdown is active, but the chassis LED could not be acquired. Release the button to cancel and inspect the hardware logs.</p>}{resetState === 'inhibited' && <p className="notice">The physical reset button is temporarily blocked while a firmware operation owns the flash lock.</p>}{resetState === 'waiting-release' && <p className="notice">Release the button once after boot before it can be armed. This prevents a stuck button from erasing settings.</p>}{status.security?.persistent_overlay_recovery_mode && <p className="warning">Persistent JFFS2 is unavailable. The switch is running with a temporary recovery overlay; changes will not survive reboot. Back up diagnostics and repair or reflash the overlay partition.</p>}</section>
...
export function PortEditor({ client, selectedPort, onSelect, onClose, config, status, poe, updatePort, updatePortMulti, diff }) {
	const ref = useRef(); const [clone, setClone] = useState(false); const { phone } = useResponsiveMode();
...
export function ConfigurationMenu({ client, auth, config, status, updateRoot, updatePort, updatePortMulti, diff, poe, hasCapability, frontTable, onFrontTableChange, portNamesExpanded, onPortNamesExpandedChange, selectedPort, onSelectPort, onApply, onDiscard, applying, connected }) {
	...
	return <ModalButton label={<CogIcon />} className="icon-button" dialogClassName={dialogClassName} title="Configuration — switch configuration and system tools">
		{({ close }) => <DialogShell title="Configuration" close={close} tabs={available} selected={tab} onSelect={setTab}>
			...
			{tab === 'display' && <div className="tab-pane"><section><h3>Display</h3><label className="checkbox-line"><input type="checkbox" checked={frontTable} disabled={phone} onChange={event => onFrontTableChange?.(event.currentTarget.checked)} /> View all ports on the front page</label>{phone && <p className="notice">The front-page All Ports view is disabled in mobile layout. Select a graphical port to open the mobile editor.</p>}<label className="checkbox-line"><input type="checkbox" checked={portNamesExpanded} onChange={event => onPortNamesExpandedChange?.(event.currentTarget.checked)} /> Keep the desktop port-name column expanded</label><p>Display preferences are stored in this browser. Only the port-number column remains pinned while horizontally scrolling.</p></section></div>}
			{tab === 'network' && <NetworkPanel client={client} config={config} status={status} updateConfig={updateRoot} diff={diff} canManage={hasCapability('network.write')} />}
			{tab === 'ports' && (phone ? <div className="tab-pane mobile-port-selector"><section><h3>Select a port</h3><div className="mobile-port-select-grid">{Object.keys(config.ports).map(port => <button key={port} onClick={() => onSelectPort?.(port)}>Port {port}</button>)}</div><p>Select a port to open the mobile accordion editor.</p></section></div> : <div className="tab-pane all-ports-pane"><AllPortsPanel client={client} config={config} status={status} poe={poe} updatePort={updatePort} updatePortMulti={updatePortMulti} diff={diff} selectedPort={selectedPort} onSelectPort={onSelectPort} showName={portNamesExpanded} onShowNameChange={onPortNamesExpandedChange} title="All Ports" /></div>)}
