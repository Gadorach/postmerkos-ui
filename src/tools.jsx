...
export function TerminalTool({ client }) {
	...
	const run = async event => {
		...
		try {
			...
			setLines(previous => [...previous, `$ ${current}`, `error: ${error.message}`]);
	...
	return <DialogButton label="terminal" title="authenticated root command window">
		{({ close }) => <div className="tool-dialog terminal-dialog">
			...
			<form className="terminal-input" onSubmit={run}>
				<span>$</span><input autoFocus value={command} onInput={event => setCommand(event.currentTarget.value)} disabled={working} />
				<button disabled={working || !command.trim()}>{working ? 'running…' : 'run'}</button>
			</form>
		</div>}
	</DialogButton>;
}
...
export function FirmwareTool({ client, connected }) {
	...
	useEffect(() => {
		...
		const poll = async () => {
			try {
				const response = await client.request('firmware_status');
				if (!stopped) setStatus(response.data);
			} catch (pollError) {
	...
	const start = async event => {
		...
		setUpload({ phase: 'hashing', progress: 0 });
		try {
			const response = await client.uploadFirmware(file, { overlay, force, onProgress: setUpload });
			setStatus({ state: 'starting', stage: 'handoff', progress: 0, message: response.data?.message });
			setUpload(null);
		} catch (uploadError) { setError(uploadError.message); setUpload(null); }
	};

	return <DialogButton label="firmware" title="upload and install firmware"
		onOpen={() => setActive(true)} onClose={() => setActive(false)}>
		{({ close }) => <div className="tool-dialog firmware-dialog">
			...
			<form onSubmit={start}>
				...
				<label>Writable-overlay policy<select value={overlay} onChange={event => setOverlay(event.currentTarget.value)}>
					<option value="preserve">Preserve settings byte-for-byte</option>
					<option value="migrate">Migrate selected settings</option>
					<option value="reset">Reset settings</option>
					<option value="image">Use overlay embedded in full image</option>
				</select></label>
...
export function BackupTool({ client, config }) {
	...
	const backup = async () => {
		setError(''); setNotice('');
		try {
			const blob = await createBackup(config, backupPassword);
			downloadBlob(blob, `postmerkos-config-${new Date().toISOString().replace(/[:.]/g, '-')}${backupPassword ? '.enc' : '.json'}`);
			setNotice(backupPassword ? 'Encrypted backup downloaded' : 'JSON backup downloaded');
		} catch (backupError) { setError(backupError.message); }
	};
	const restore = async () => {
		setWorking(true); setError(''); setNotice('');
		try {
			const restored = await readBackup(restoreFile, restorePassword);
			await client.request('replace_config', restored, { timeout: 30000 });
			setNotice('Configuration restored and applied');
	...
	return <DialogButton label="backup" title="back up or restore configuration">
		{({ close }) => <div className="tool-dialog backup-dialog">
			...
			<section><h3>Export</h3>
				<label>Optional password<input type="password" value={backupPassword} onInput={event => setBackupPassword(event.currentTarget.value)} /></label>
				<button onClick={backup} disabled={!config}>Download backup</button>
			</section>
			<section><h3>Restore</h3>
				<label>Backup file<input type="file" onChange={event => setRestoreFile(event.currentTarget.files?.[0] ?? null)} /></label>
				<label>Password for encrypted backup<input type="password" value={restorePassword} onInput={event => setRestorePassword(event.currentTarget.value)} /></label>
				<button onClick={restore} disabled={!restoreFile || working}>{working ? 'restoring…' : 'Validate and restore'}</button>
			</section>
...
export function AccountTool({ client, auth }) {
	...
	const change = async event => {
		...
		try {
			await client.request('password_change', { username, current_password: current, new_password: replacement }, { timeout: 20000 });
			setCurrent(''); setReplacement(''); setConfirmation(''); setNotice(`Password updated for ${username}`);
